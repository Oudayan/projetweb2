...
if ($_SESSION["type"] == 3 || $_SESSION["type"] == 2) : ?>
    ...
    <div class="d-flex container">
        ...
        <div class="nav flex-column nav-pills m-1" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            <a class="nav-link active" id="v-pills-home-tab" data-toggle="pill" href="#v-pills-home" role="tab" aria-controls="v-pills-home" aria-selected="true">Afficher les membres</a>
            <a class="nav-link" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-selected="false">Afficher les locations</a>
            <a class="nav-link" id="v-pills-messages-tab" data-toggle="pill" href="#v-pills-messages" role="tab" aria-controls="v-pills-messages" aria-selected="false">Afficher les achats</a>
            <a class="nav-link" id="v-pills-settings-tab" data-toggle="pill" href="#v-pills-settings" role="tab" aria-controls="v-pills-settings" aria-selected="false">Afficher les messageries </a>
        </div>
                    ...
                    <?php foreach ($donnees['membres'] as $membre) { 
                        ...
                        <tr class="<?= $membre->getMembreActif() ? "" : "text-danger" ?> <?= $membre->getTypeUtilisateur() == 1 ? "" : "text-success" ?>">
                            ...
                            <?php if (!$membre->getMembreValide()) { ?>
                                <td><a href="index.php?Admin&action=validerMembre&membre_id=<?= $membre->getMembreId();?>" class="btn btn-outline-success m-1">Valider</a> </td>
                            <?php  }
                            if($membre->getMembreValide() && $membre->getTypeUtilisateur() !=3 && $membre->getMembreActif() && ($_SESSION["type"] == 3 || ($_SESSION["type"]==2 && $membre->getTypeUtilisateur()== 1))) { ?>
                                <td><a href="index.php?Admin&action=bannirMembre&membre_id=<?= $membre->getMembreId();?>" class="btn btn-outline-danger m-1">Bannir</a></td>
                            <?php  }
                            if (!$membre->getMembreActif()) { ?>
                                <td><a href="index.php?Admin&action=reactiverMembre&membre_id=<?= $membre->getMembreId();?>" class="btn btn-outline-warning m-1">Dé-bannir</a> </td>
                            <?php  }
                            if ($membre->getTypeUtilisateur() == 1 && $_SESSION["type"] == 3 && $membre->getMembreActif() && $membre->getMembreValide() ) { ?>
                                <td><a href="index.php?Admin&action=promouvoirMembre&membre_id=<?= $membre->getMembreId();?>" class="btn btn-outline-primary m-1">Promouvoir</a></td>
                            <?php  }if ( $_SESSION["type"] == 3  && $membre->getTypeUtilisateur()== 2 ) { ?>
                                <td><a href="index.php?Admin&action=demouvoirMembre&membre_id=<?= $membre->getMembreId();?>" class="btn btn-outline-info m-1">Rétrograder</a> </td>
                            <?php  } ?>
                        </tr>
                        <?php }
                    } ?>
